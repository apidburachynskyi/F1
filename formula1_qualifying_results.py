    ...
    for eventname in eventname_list:
        for session in session_list:
            ...
            if eventname == eventname_list[0] and session == session_list[0] and year == year_list[0]:
                qualifying_results_df = pd.DataFrame(session_results)
            else:
                qualifying_results_df = pd.concat([qualifying_results_df,pd.DataFrame(session_results)])

            print(qualifying_results_df.shape)

qualifying_results_df.to_csv('csv/qualifying_results_df.csv')
